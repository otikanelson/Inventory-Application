...
export interface TourStep {
  ...
  action?: 'navigate' | 'highlight'; // What to do at this step
}

interface MultiScreenTourProps {
  steps: TourStep[];
  onComplete: () => void;
  visible: boolean;
...
export const MultiScreenTour: React.FC<MultiScreenTourProps> = ({
  steps,
  onComplete,
  visible,
  currentScreen,
  ...
  useEffect(() => {
    // Check if we need to navigate for current step
    const step = steps[currentStep];
    if (step && step.screen && step.screen !== currentScreen && !isNavigating) {
      // We're on wrong screen, need to navigate
      handleNavigation(step.screen);
    }
  ...
  const handleNavigation = async (screen: string) => {
    ...
    setTimeout(() => {
      ...
      // Wait for navigation to complete, then fade back in
      setTimeout(() => {
  ...
  const handleNext = () => {
    if (currentStep < steps.length - 1) {
      ...
      // If next step requires navigation, it will happen in useEffect
    } else {
      handleComplete();
    }
  };
  ...
  // Don't show overlay if we're navigating or on wrong screen
  if (isNavigating || (step.screen && step.screen !== currentScreen)) {
    return null;
  }
